#' Internal: Storage for the current API session
#' @noRd
...
#' @export
formr_store_keys <- function(host = "https://formr.org", client_id = NULL, client_secret = NULL, access_token = NULL) {
	if (!requireNamespace("keyring", quietly = TRUE)) stop("Package 'keyring' is required.")
	
	service_name <- paste0("formr_", host)
	
	if (!is.null(access_token)) {
		keyring::key_set_with_value(service = service_name, username = "access_token", password = access_token)
...
#' @export
formr_api_authenticate <- function(host = "https://formr.org", client_id = NULL, client_secret = NULL, access_token = NULL) {
	...
	if (!is.null(access_token)) {
		...
		res <- httr::POST(token_url, body = list(
			client_id = client_id, client_secret = client_secret, grant_type = "client_credentials"
		), encode = "form")
		
		if (httr::status_code(res) >= 400) stop("OAuth Error: ", httr::content(res, "text"))
		
		token <- httr::content(res)$access_token
		.formr_current_session$set(list(base_url = httr::parse_url(host), token = token))
...
#' @noRd
formr_api_request <- function(endpoint, method = "GET", body = NULL, query = NULL, api_version = "v1", encode = NULL) {
	session <- formr_api_session()
	if (is.null(session)) stop("Not authenticated. Run formr_api_authenticate().")
	...
	if (is.null(encode)) {
		encode <- "json"
		if (any(sapply(body, inherits, "form_file"))) encode <- "multipart"
	}
	
	if (method == "GET") req <- httr::GET(url, query = query, auth)
	else if (method == "POST") req <- httr::POST(url, body = body, encode = encode, auth)
	else if (method == "PUT") req <- httr::PUT(url, body = body, encode = encode, auth)
	else if (method == "PATCH") req <- httr::PATCH(url, body = body, encode = encode, auth)
	else if (method == "DELETE") req <- httr::DELETE(url, auth)
	
	if (httr::http_type(req) != "application/json") {
		stop(sprintf("API Error: Expected JSON, got %s.\nPreview: %s", 
								 httr::http_type(req), substr(httr::content(req, "text"), 1, 200)))
	}
	
	if (httr::status_code(req) >= 400) {
		stop(sprintf("API Error (%s): %s", httr::status_code(req), httr::content(req, "text")))
	}
